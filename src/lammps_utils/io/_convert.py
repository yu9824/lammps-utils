...
def data2pdb(
    ...
    pdb_atoms = "\n".join(
        "ATOM  "
        + df_atoms.index.map(lambda x: "{:>5}".format(x))  # 原子の通し番号
        + " "
        + df_atoms["symbol"].map(lambda x: "{:^4}".format(x))  # 原子名
        + " "
        + "MOL"  # 残基名
        + " "
        + (df_atoms["mol"] + ord("A") - 1).map(lambda x: chr(x))  # 鎖名
        # + "A"
        + "   1    "  # 残基番号
        # + "   "
        # + df_atoms["mol"].map(lambda x:"{:>4}".format(x))
        # + "    "
        + (
            df_atoms[["x", "y", "z"]]
            - np.array([_bound[0] for _bound in cell_bounds])
        )
        .map(lambda x: "{:>8.3f}".format(x))
        .sum(axis=1)  # x,y,z座標
        + "  1.00"  # 占有率
        + "  0.00"  # 温度因子?
        + "          "
        + df_atoms["symbol"].map(lambda x: "{:>2}".format(x))  # 元素記号
        + "  "  # 電荷
    )
