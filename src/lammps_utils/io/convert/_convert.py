...
from lammps_utils.io.dataframe._dataframe import load_data
...
def data2gro(
    ...
    cell_size = (
        np.array([_max - _min for _min, _max in cell_bounds])
        / 10  # angstrom to nm
    )
    ...
    content_gro = (
        ...
        + "\n".join(
            (
                df_atoms["mol"].map(lambda x: "{:>5}".format(x))
                + "MOL01"
                + df_atoms["symbol"].map(lambda x: "{:>5}".format(x))
                + df_atoms.index.map(lambda x: "{:>5}".format(x))
                + (
                    (
                        df_atoms[["x", "y", "z"]] / 10  # angstrom to nm
                    )
